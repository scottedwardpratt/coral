...
CCF2S_Minuit_Blast::CCF2S_Minuit_Blast(CSourceCalc *scset,C3DArray *cexpset,
  ...
  pars[0].Set("lambda",(sourcecalc->spars).getD("lambda",1),1.0,0,2.0);
  pars[1].Set("R",(sourcecalc->spars).getD("R",13.0),1.0,0.0,20.0);
  pars[2].Set("Tau",(sourcecalc->spars).getD("Tau",12.0),1.0,0.0,20.0);
  pars[3].Set("DelTau",(sourcecalc->spars).getD("DelTau",5),1.0,0.0,20.0);
  InitMinuit();
  //FixPar(6);
  //FixPar(5);
  //FixPar(4);
