...
CCF2SFit_Blast::CCF2SFit_Blast(CSourceCalc *scset,C3DArray *cexp3Dset,C3DArray *cerror3Dset, C3DArray *ctheory3Dset,CCHArray *ctheoryset,CCHArray *sourceset,CKernel *kernelset){
	...
	AddPar("lambda",(sourcecalc->spars).getD("lambda",1),
		0.01,0.0,1.5);
	AddPar("R",(sourcecalc->spars).getD("R",13),
		0.05,5.0,20.0);
	AddPar("Tau",(sourcecalc->spars).getD("Tau",12),
		0.05,5.0,20.0);
	AddPar("DelTau",(sourcecalc->spars).getD("DelTau",5),
		0.05,0.0,20.0);
