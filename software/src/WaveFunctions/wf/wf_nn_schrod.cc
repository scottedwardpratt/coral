...
double CWaveFunction_nn_schrod::CalcPsiSquared(int iq,double r,double ctheta){
	...
	//delpsiS0=0.0;
	if(q<3.0)
		delpsiS1J0=delpsiS1J1=delpsiS1J2=0.0;
...
void CWaveFunction_nn_schrod::schrodinger(int ichannel,int iq){
	...
	if(ichannel==0){
		...
		L=1; 
		if(ichannel==1){
			snprintf(pname,10,"3P0");
		}
		else if(ichannel==2){
			snprintf(pname,10,"3P1");	
		}
		else{
			snprintf(pname,10,"3C2");
		}
	}	
	...
	for(ir_schrod=1;ir_schrod<=nrmax_schrod;ir_schrod++){
		...
		if(ir2<=nr){
			...
			snprintf(message,CLog::CHARLENGTH,"OOPS, out of bounds, shouldn't be here, ir1=%d, nr=%d\n",ir1,nr);
			CLog::Fatal(message);
		}
	}
	//if(ichannel==0)
