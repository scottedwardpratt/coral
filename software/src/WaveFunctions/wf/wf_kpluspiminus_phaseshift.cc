...
CWaveFunction_kpluspiminus_phaseshift::CWaveFunction_kpluspiminus_phaseshift(string  parsfilename) : CWaveFunction() {
  ...
  ellmax=1;
  InitArrays();
  snprintf(message,CLog::CHARLENGTH,"Arrays Initialized\n");
  CLog::Info(message);

  ell[0]=1;

  InitWaves();
  snprintf(message,CLog::CHARLENGTH,"Partial Waves Initialized\n");
  CLog::Info(message);
...
void CWaveFunction_kpluspiminus_phaseshift::get_phaseshifts_kpluspiminus(){
  ...
  snprintf(message,CLog::CHARLENGTH,"mtest=%g =? 891.66, q0=%g\n",mtest,q0);
  CLog::Info(message);
  for(iq=0;iq<=nqmax;iq++){
    q=qarray[iq];
    ...
    snprintf(message,CLog::CHARLENGTH,"q=%g, m=%g, delta=%g, ddeltadq=%g =? %g\n",
