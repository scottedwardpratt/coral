...
int main(int argc,char *argv[]){
  ...
  FILE *fptr;
	...
	fptr=fopen("delta_2P_12.txt","r");
	for(iq=0;iq<NQMAX;iq++){
		fscanf(fptr,"%lf %lf",&elab[iq],&delta);
		qarray[iq]=(MP*MD/(MP+MD))*sqrt(2.0*elab[iq]/MP);
		fprintf(output,"%lf\n",qarray[iq]);
		deltaS12[iq]+=(1.0/3.0)*delta;
	}
	fclose(fptr);
	fclose(output);
	
	fptr=fopen("delta_2P_32.txt","r");
	for(iq=0;iq<NQMAX;iq++){
		fscanf(fptr,"%lf %lf",&elab[iq],&delta);
		deltaS12[iq]+=(2.0/3.0)*delta;
	}
	fclose(fptr);
	
	fptr=fopen("delta_4P_12.txt","r");
	for(iq=0;iq<NQMAX;iq++){
		fscanf(fptr,"%lf %lf",&elab[iq],&delta);
		deltaS32[iq]+=(1.0/6.0)*delta;
	}
	fclose(fptr);
	
	fptr=fopen("delta_4P_32.txt","r");
	for(iq=0;iq<NQMAX;iq++){
		fscanf(fptr,"%lf %lf",&elab[iq],&delta);
		deltaS32[iq]+=(2.0/6.0)*delta;
	}
	fclose(fptr);
	
	fptr=fopen("delta_4P_52.txt","r");
	for(iq=0;iq<NQMAX;iq++){
		fscanf(fptr,"%lf %lf",&elab[iq],&delta);
		deltaS32[iq]+=(3.0/6.0)*delta;
	}
	fclose(fptr);
